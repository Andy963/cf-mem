import type { ClaimApplicability, ClaimInput, ClaimProvenance, ClaimStatus, ClaimType, ScopeKind } from "../memory/claims";
...
export async function fetchActiveClaimByIdentity(
  db: D1Database,
  projectId: string,
  claim: Pick<ClaimInput, "scopeKind" | "scopeId" | "type" | "subject" | "memoryKey" | "workspaceId">,
): Promise<StoredClaimRow | null> {
  const result = await db
    .prepare(
      `SELECT ${CLAIM_COLUMNS} FROM memory_claims WHERE project_id = ? AND scope_kind = ? AND scope_id = ? AND type = ? AND subject = ? AND memory_key = ? AND COALESCE(workspace_id, '') = COALESCE(?, '') AND status = 'active'`,
    )
    .bind(projectId, claim.scopeKind, claim.scopeId, claim.type, claim.subject, claim.memoryKey, claim.workspaceId)
    .first<StoredClaimRow>();
  return result ?? null;
}
...
function insertClaimStatement(
  ...
  return db
    .prepare(
      "INSERT INTO memory_claims (id, project_id, scope_kind, scope_id, type, subject, memory_key, value_json, canonical_text, status, provenance, confidence, valid_from, valid_until, superseded_by, applicability, workspace_id, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NULL, ?, ?, ?, ?)",
    )
    .bind(
      claimId,
      projectId,
...
export async function fetchContextClaims(
  db: D1Database,
  projectId: string,
  options: { userId: string | null; sessionId: string | null; types: ClaimType[] | null; limit: number },
): Promise<StoredClaimRow[]> {
  const now = Date.now();
  const scopes: Array<[ScopeKind, string, number]> = [["project", projectId, 1]];
  ...
  const result = await db
    .prepare(
      `SELECT ${CLAIM_COLUMNS} FROM memory_claims WHERE project_id = ? AND status = 'active' AND (valid_from IS NULL OR valid_from <= ?) AND (valid_until IS NULL OR valid_until > ?) AND (${scopeConditions})${typeConditions} ORDER BY ${priority} ASC, updated_at DESC LIMIT ?`,
    )
    .bind(projectId, now, now, ...scopeBindings, ...typeBindings, options.limit)
    .all<StoredClaimRow>();
  return result.results;
}
...
export async function fetchGlobalProfileClaims(
  ...
  const result = await db.prepare(
    `SELECT ${CLAIM_COLUMNS} FROM memory_claims WHERE project_id = ? AND scope_kind = 'user' AND scope_id = ? AND type = 'preference' AND applicability = 'global' AND status = 'active' AND (valid_from IS NULL OR valid_from <= ?) AND (valid_until IS NULL OR valid_until > ?) ORDER BY updated_at DESC LIMIT ?`,
  ).bind(projectId, userId, now, now, limit).all<StoredClaimRow>();
  return result.results;
}
...
export async function fetchWorkspaceProfileClaims(
  ...
  const now = Date.now();
  const result = await db.prepare(
    `SELECT ${CLAIM_COLUMNS} FROM memory_claims WHERE project_id = ? AND scope_kind = 'user' AND scope_id = ? AND applicability = 'workspace' AND workspace_id = ? AND status = 'active' AND (valid_from IS NULL OR valid_from <= ?) AND (valid_until IS NULL OR valid_until > ?) ORDER BY updated_at DESC LIMIT ?`,
  ).bind(projectId, userId, workspaceId, now, now, limit).all<StoredClaimRow>();
  return result.results;
}
...
export async function fetchOwnerClaims(
  ...
  const active = await db
    .prepare(
      `SELECT ${CLAIM_COLUMNS} FROM memory_claims WHERE project_id = ? AND scope_kind = 'user' AND scope_id = ? AND status = 'active' ORDER BY updated_at DESC LIMIT ?`,
    )
    .bind(projectId, ownerId, activeLimit)
    .all<StoredClaimRow>();
  const inactive = await db
    .prepare(
      `SELECT ${CLAIM_COLUMNS} FROM memory_claims WHERE project_id = ? AND scope_kind = 'user' AND scope_id = ? AND status != 'active' ORDER BY updated_at DESC LIMIT ?`,
    )
    .bind(projectId, ownerId, inactiveLimit)
    .all<StoredClaimRow>();
