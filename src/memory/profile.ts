...
export const DEFAULT_EXTRACTOR_INSTRUCTIONS = [
  ...
  "A preference or instruction MUST explicitly define a reusable future assistant behavior, engineering standard, naming convention, or workflow constraint.",
  "DISTINGUISH ONE-OFF TASKS FROM ENDURING RULES: Reject action logs, session summaries, and one-off task parameters (e.g., '对传输的歌曲进行命名改进', '修复了某个解析错误', '修改了当前目录下的文件'). Accept only enduring behavioral constraints, coding standards, and workflow rules.",
  ...
  "When a user corrects, criticizes, or expresses frustration about the assistant's behavior: extract the underlying general rule if it implies a reusable constraint (e.g., '版本号必须是三位数字'). If the correction is strictly scoped to one-off task data at hand (e.g., '这首歌名字改错了'), DO NOT extract a general rule.",
...
export const DEFAULT_VERIFIER_INSTRUCTIONS = [
  ...
  "'Always reply in Chinese' is a preference. 'I prefer concise answers' is a preference. 'Use pytest in this repo' is a preference. 'Default to developing on dev branch and merge to main' is an instruction.",
  "User corrections and complaints about general assistant behavior encode explicit rules — accept them as preferences or instructions. '版本号应该是三位数字' is a preference.",
  "REJECT action summaries and event logs: reject any candidate that merely summarizes what happened in the session or describes a one-off task (e.g., '对文件进行了重命名', '对传输的歌曲进行命名改进', '处理了某批数据').",
  "REJECT vague and non-actionable statements: reject any candidate lacking clear deterministic execution rules (e.g., '改进命名', '注意代码质量').",
  "'I think X is better than Y' is an opinion — reject. 'I currently use X' or 'I am on branch dev' is factual current_state — reject. 'X is inefficient' is an evaluation — reject.",
  "Reject beliefs, subjective evaluations, factual state descriptions, temporary tasks, and anything that does not alter future assistant behavior.",
  "Accept explicit, enduring rules, coding standards, naming conventions, and workflow constraints directly supported by cited user evidence.",
