...
BEGIN
  INSERT INTO memory_segment_vector_jobs (
    ...
    attempt_count, last_error, next_attempt_at, lease_token, lease_expires_at, created_at, updated_at
  ) VALUES (
    NEW.project_id, NEW.id, 1,
    CASE WHEN NEW.deletion_state = 'active' THEN 'upsert' ELSE 'delete' END,
    NEW.updated_at, 'pending', 0, NULL, NEW.updated_at, NULL, NULL, NEW.updated_at, NEW.updated_at
  )
...
BEGIN
  INSERT INTO memory_segment_vector_jobs (
    ...
    'pending', 0, NULL, OLD.updated_at, NULL, NULL, OLD.updated_at, OLD.updated_at
  )
  ON CONFLICT (project_id, segment_id) DO UPDATE SET
    revision = memory_segment_vector_jobs.revision + 1,
    operation = 'delete',
