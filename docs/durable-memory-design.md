...
网页抓取只通过已认证的 Tavily relay 完成。Worker 仅检查 URL 基础格式，不执行目标地址的直接
网络请求；DNS 解析、重定向和私网地址策略由 relay 负责，因此 Worker 的出站抓取路径不再
提供 DNS rebinding 入口。relay 不可用时，网页引用抓取失败并被跳过。
